...
def index2(request):
  ...
  context = {
    ...
    "bospenplayers": Player.objects.filter(curr_team__location="Boston") &
 Player.objects.filter(curr_team__team_name="Penguins"),
    ...
    "lopezfootballplayer": Player.objects.filter(curr_team__league__name="American Conference of Amateur Football") &
Player.objects.filter(last_name="Lopez"),
    ...
    "floresnot": Player.objects.filter(last_name='Flores').exclude(curr_team__team_name='Roughriders') & Player.objects.exclude(curr_team__location='Washington'),
